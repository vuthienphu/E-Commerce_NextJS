...
const queryString = params.toString();
 ...
 const { data: products, error: productError, isLoading: productLoading } = useSWR<Product[]>(
        ...
        <aside className="sidebar">
          ...
          <div className="category-panel">
            ...
            <div className="category-list">
              {categories.map((item)=>(<div key={item.id} className='category-item'>{item.name}</div>))}
            </div>
          </div>

          {/* Other filters */}
          <FilterSection title="Size" options={sizes} selectedValues={selectedSizes} onChange={handleSizeChange} />
        ...
        {products.map((item) => (
          <div key={item.id} className="sell-product-item">
           ...
           <span className="info">{item.name}</span>
            ...
            <span className="price">${item.original_price * (1 - item.discount_percent / 100)}</span>
